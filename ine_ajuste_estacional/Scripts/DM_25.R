...
#             Tema: Cálculos - Generacion ajuste para serie de mujeres desocupadas de 25 años o más
...
X13_dm25 <- seas(dm25,
                 ...
                 regression.variables = c("rp2019.dec-2020.jun","ao2020.feb"),
...
f <- as.data.frame(series(X13_dm25, "forecast.forecasts"))
names(f)[1] <- "dm25"
names(f)[2] <- "dm25_lower"
names(f)[3] <- "dm25_upper"

summary(X13_dm25)

# Gráfico comparativo serie original y ajustada
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Comparación series DM 25 ",tm,".png"), width = 700, height = 700)
plot(X13_dm25, 
     main = "DM 25 serie original y ajustada", 
     trend = TRUE, 
     xlab = "Periodo", 
     ylab = "Stock ajustado")
...
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Componente estacional por mes DM 25",tm,".png"), width = 700, height = 700)
monthplot(X13_dm25, 
          main = "Componente estacional (ratio SI) para DM 25")
legend("topright",c("SI componente","Componente estacional"),
       fill=c("blue","red"))
dev.off() 

# Gráfico de resifuos del regARIMA
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Residuos regARIMA DM 25 ",tm,".png"), width = 700, height = 700)
residplot(X13_dm25, 
          main = "Residuos de regArima para DM 25",
          xlab = "Periodo")
dev.off()
...
seasonal <- as.data.frame(X13_dm25[["data"]])
names(seasonal)[3] <- "dm25_sa"
names(seasonal)[4] <- "dm25_tc"
