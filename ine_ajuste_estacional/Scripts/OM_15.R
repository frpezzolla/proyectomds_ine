...
#             Tema: Cálculos - Generacion ajuste para serie de mujeres ocupadas entre 15 y 24 años
...
f <- as.data.frame(series(X13_om15, "forecast.forecasts"))
names(f)[1] <- "om15"
names(f)[2] <- "om15_lower"
names(f)[3] <- "om15_upper"

#static(X13_om15, coef = TRUE, test = TRUE)
summary(X13_om15)

# Gráfico comparativo serie original y ajustada
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Comparación series OM 15-24 ",tm,".png"), width = 700, height = 700)
plot(X13_om15, 
     main = "OM 15-24 serie original y ajustada", 
     trend = TRUE, 
     xlab = "Periodo", 
     ylab = "Stock ajustado")
...
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Componente estacional por mes OM 15-24",tm,".png"), width = 700, height = 700)
monthplot(X13_om15, 
          main = "Componente estacional (ratio SI) para OM 15-24")
legend("topright",c("SI componente","Componente estacional"),
       fill=c("blue","red"))
dev.off() 

# Gráfico de resifuos del regARIMA
png(paste0("//buvmfswinp01/ene2/Ajuste estacional/Salidas/Gráficos/", yy,"/", ntm," ",tm,"/Residuos regARIMA OM 15-24 ",tm,".png"), width = 700, height = 700)
residplot(X13_om15, 
          main = "Residuos de regArima para OM 15-24",
          xlab = "Periodo")
dev.off()
...
seasonal <- as.data.frame(X13_om15[["data"]])
names(seasonal)[3] <- "om15_sa"
names(seasonal)[4] <- "om15_tc"
